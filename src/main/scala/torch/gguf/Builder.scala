...
trait Builder extends Cloneable {
  /**
  ...
  def setAlignment(newAlignment: Int): Builder = {
    if (newAlignment < 0 || Integer.bitCount(newAlignment) != 1) throw new IllegalArgumentException("alignment must be a power of 2 but was " + newAlignment)
   ...
   */
  def getMetadataKeys: util.Set[String]
   ...
   */
  def getTensors: util.Collection[TensorInfo]
